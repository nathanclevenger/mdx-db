...
export function validateSearchOptions(options) {
    ...
    if (options.vector.length < VECTOR_VALIDATION.MIN_DIMENSIONS ||
        ...
        throw new VectorError(`Vector dimensions must be between ${VECTOR_VALIDATION.MIN_DIMENSIONS} and ${VECTOR_VALIDATION.MAX_DIMENSIONS}`);
    }
    ...
    if (options.threshold !== undefined) {
        if (typeof options.threshold !== 'number' ||
            ...
            throw new VectorError(`threshold must be between ${VECTOR_VALIDATION.MIN_THRESHOLD} and ${VECTOR_VALIDATION.MAX_THRESHOLD}`);
        }
    }
